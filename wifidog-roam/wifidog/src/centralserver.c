...
void roam_server_request(t_authresponse * authresponse,  const char *mac, int type)
{
    s_config *config = config_get_config();
    int sockfd;
	...
	if(type == 1)
	{
	    sprintf(type_str,"add");
	}
	else
	{
		sprintf(type_str,"del");
	}


	snprintf(request, sizeof(request) - 1,
				 "GET %s%sgw_id=%s&mac=%s&type=%s HTTP/1.0\r\n"
				 "User-Agent: WiFiDog %s\r\n"
				 "Host: %s\r\n"
				 "\r\n",
				 auth_server->authserv_path,
				 auth_server->authserv_roam_script_path_fragment,
				 config->gw_id,
			     mac,
			     type_str);
...
#endif
			if (NULL == res) {
				debug(LOG_ERR, "There was a problem when roam mac the auth server!");
			} else if (strstr(res, "roamed") == 0) {
				debug(LOG_WARNING, "Auth server did NOT roam the mac!");
		
				free(res);
			} else {
				debug(LOG_DEBUG, "Auth Server Says: roamed");
