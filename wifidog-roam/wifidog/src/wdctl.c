...
//added by chenang@20210312
static void wdctl_roam(int type);
...
usage(void)
{
	...
	fprintf(stdout, "  roam mac          roam mac on local\n");
	/*added by chengang@20210318*/
	fprintf(stdout, "  delroam mac       delete roam mac on local\n");
    fprintf(stdout, "\n");
}
...
parse_commandline(int argc, char **argv)
{
	...
	/*Added by chengang@20210312*/
	else if(strcmp(*(argv + optind), "roam") == 0) 
	{
		config.command = WDCTL_ROAM;
        if ((argc - (optind + 1)) <= 0) {
            fprintf(stderr, "wdctl: Error: You must specify  a Mac address to roam\n");
            usage();
            exit(1);
        }
        config.param = strdup(*(argv + optind + 1));
	...
	else if(strcmp(*(argv + optind), "delroam") == 0) 
	{
		config.command = WDCTL_ROAMDEL;
        if ((argc - (optind + 1)) <= 0) {
            fprintf(stderr, "wdctl: Error: You must specify  a Mac address to roam del\n");
            usage();
            exit(1);
        }
        config.param = strdup(*(argv + optind + 1));
...
static void wdctl_roam(int type)
{
    ...
    if(type == ROAM_ADD)
	{
		strncpy(request, "roam ", 64);
	}
	else
	{
		strncpy(request, "delroam ", 64);
	}
    strncat(request, config.param, (64 - strlen(request) - 1));
    strncat(request, "\r\n\r\n", (64 - strlen(request) - 1));
    ...
    if (strcmp(buffer, "Yes") == 0) {
        fprintf(stdout, "Connection %s successfully roam. type(%d)\n", config.param,type);
    } else if (strcmp(buffer, "No") == 0) {
        fprintf(stdout, "Connection %s was not active.type(%d)\n", config.param,type);
    } else {
        fprintf(stderr, "wdctl: Error: WiFiDog sent an abnormal for roam " "reply.\n");
    }
