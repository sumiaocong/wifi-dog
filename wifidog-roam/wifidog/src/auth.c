...
void roam_client(const char *mac,int type)
{
	t_authresponse authresponse;
    const s_config *config = config_get_config();
    //fw_deny(client);
    ...
    if (config->auth_servers != NULL) 
	{
		roam_server_request(&authresponse,mac,type);
        if (authresponse.authcode == AUTH_ERROR)
    	{
        	debug(LOG_WARNING, "Auth server error when reporting logout");
    	}
		else if(authresponse.authcode == AUTH_ALLOWED)
		{
			debug(LOG_INFO, "auth roam mac --(%s) type(%d)", mac,type);
			if(type == 1)
			{
				auth_roam_client_trust(mac);
			}
			else
			{
				auth_roam_del_client_trust(mac);
			}
		}
    }
}
