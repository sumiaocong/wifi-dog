...
typedef enum {
    ...
    oAuthServRoamScriptPathFragment,
    oHTTPDMaxConn,
    oHTTPDName,
    oHTTPDRealm,
    oHTTPDUsername,
...
static const struct {
	...
	//added by chengang@20210317
	"roamscriptpathfragment", oAuthServRoamScriptPathFragment}, {
...
parse_auth_server(FILE * file, const char *filename, int *linenum)
{
	...
	//added by chengang@20210317
	roamscriptpathfragment = safe_strdup(DEFAULT_AUTHSERVROAMPATHFRAGMENT);
        ...
        if (strlen(p1) > 0) {
            ...
            *p2 = '\0';
            ...
            case oAuthServAuthScriptPathFragment:
                ...
                break;
			//added by chengang@20210317
			case oAuthServRoamScriptPathFragment:
                free(roamscriptpathfragment);
                roamscriptpathfragment = safe_strdup(p2);
                break;
	...
	//added by chengang@20210317
	new->authserv_roam_script_path_fragment = roamscriptpathfragment;
...
validate_popular_servers(void)
{
    if (config.popular_servers == NULL) {
		...
		//deleted for bug2018 by chengang@20210324
