...
/*Added by chengang@20210321*/
pthread_mutex_t roam_list_mutex = PTHREAD_MUTEX_INITIALIZER;
...
//added by chengang@20210312
t_trusted_mac *roam_list_find(const char *mac)
{
	t_trusted_mac *p;
	s_config *config = config_get_config();
	for (p = config->roamedmaclist; p != NULL; p = p->next)
	{
        if (0 == strcmp(p->mac, mac))
        {
		    debug(LOG_INFO, "roam_list_find found a mac");
	        return p;
        }
            //p = p->next;
	}
...
t_trusted_mac *trust_list_find(const char *mac)
{
	t_trusted_mac *p;
	s_config *config = config_get_config();
	for (p = config->trustedmaclist; p != NULL; p = p->next)
	{
        if (0 == strcmp(p->mac, mac))
        {
			debug(LOG_INFO, "trust_list_find found a mac");
            return p;
        }
        //p = p->next;
	}
...
int add_to_roamed_mac_list(const char *mac)
{
	t_trusted_mac *p;
	s_config *config = config_get_config();
	if (config->roamedmaclist == NULL) 
	{
        config->roamedmaclist = safe_malloc(sizeof(t_trusted_mac));
        config->roamedmaclist->mac = safe_strdup(mac);
        config->roamedmaclist->next = NULL;
	} 
	else 
	{
        int skipmac;
        /* Advance to the last entry */
        p = config->roamedmaclist;
        skipmac = 0;
        ...
        if (!skipmac) {
            ...
            debug(LOG_ERR,
                  "MAC address [%s] already on roamed list.",
                  mac);
			return 0;
        }
   }
	debug(LOG_INFO,"Added a mac to roamedlist!");
	return 1;
}

//added by chengang@20210318
int del_from_roamed_mac_list(const char *mac)
{
	t_trusted_mac *p;
	t_trusted_mac *tmp;
	s_config *config = config_get_config();
	...
	if (config->roamedmaclist == NULL) 
	{
		debug(LOG_ERR,"roamed list is null.");
		return 0;
	} 
    ...
    if (0 == strcmp(p->mac, mac)) 
	{
	    ...
	    if(p->next != NULL)
	    {
	        debug(LOG_INFO,"next node is null");
	    	config->roamedmaclist = p->next;
	    }
		else
		{
			config->roamedmaclist = NULL;
		}
		FREE(p);
    }
	else 
	{
		...
		{
			tmp=p->next;
			debug(LOG_INFO,"tmp-mac (%s)",tmp->mac);
            if (0 == strcmp(tmp->mac, mac)) 
			{
				find = 1;
	           //move dst next node  to up layer
				//if(tmp->next != NULL)
				//{
            	p->next = tmp->next;
				//}
    ...
    if (find)
	{
		debug(LOG_INFO,"delete a roam mac ");
		return 1;
    } 
	else 
	{
        debug(LOG_ERR,
              "MAC address [%s] not found from roamed list.",
              mac);
		return 0;
    }
