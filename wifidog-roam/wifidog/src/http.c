...
http_callback_404(httpd * webserver, request * r, int error_code)
{
	...
	FILE * fp;
	char ip[80];
	debug(LOG_DEBUG, "enter http_callback_404 ");
...
//FIX ME 
    fp = popen("ip r|grep default|awk -F' ' '{print $3}'", "r");
    fgets(ip, sizeof(ip), fp);
    //printf("%s", buffer);
    pclose(fp);
	char *buf;
	safe_asprintf(&buf,
				  "<p align=\"center\"> forbidden!</p>"
				  "<p>本路由器设备限制在中国移动宽带网络下使用，请切换网络重试</p>"
				  "<p>设备当前网络ip地址为:%s. </p>",
				 // "<p>In a couple of minutes please <a href='%s'>click here</a> to try your request again.</p>",
				  ip);
	
	send_http_page(r, "CMCC", buf);
	free(buf);
    ...
    if (!is_online()) {
        ...
        char *buf;
        safe_asprintf(&buf,
                      "<p> Authentication server unreachable !</p>"
                      "<p>Please check network or config of server. </p>",
                     // "<p>In a couple of minutes please <a href='%s'>click here</a> to try your request again.</p>",
                      tmp_url);

        send_http_page(r, "Error!", buf);
        free(buf);
        debug(LOG_INFO, "Sent %s an apology since auth server not online - no point sending them to auth server",
              r->clientAddr);
    } else {
