...
//added by chengang@20210311
static void wdctl_roam(int, const char *);
int auth_roam_client_trust(const char *mac);
static void wdctl_roamdel(int, const char *);
int auth_roam_del_client_trust(const char *mac);
...
thread_wdctl_handler(void *arg)
{
	...
	//added by chengang@20210311
	else if(strncmp(request, "roam", 4) == 0)
	{
		wdctl_roam(fd,(request + 5));
	}
	//added by chengang@20210317
	else if(strncmp(request, "delroam", 7) == 0)
	{
		wdctl_roamdel(fd,(request + 8));
	}
	else {
...
int auth_roam_client_trust(const char *mac)
{
	...
	if (add_to_roamed_mac_list(mac)) 
	{
		debug(LOG_INFO, "add a roamd mac ok!");
		if (!iptables_fw_access_roam(FW_ACCESS_ALLOW,mac)){
	        rc = 0;
		}
	}
...
static void wdctl_roam(int fd, const char *arg)
{
	
	debug(LOG_INFO, "Entering wdctl_roam...");
	if(!is_valid_mac(arg))
	{
		debug(LOG_ERR, "wdctl_roam input a invalid MAC address!");
    ...
    if ((client_list_find_by_mac(arg)) != NULL)
	{
		debug(LOG_DEBUG, " not need roam.");
		UNLOCK_CLIENT_LIST();
		write_to_socket(fd, "No", 2);	/* Error handling in fucntion sufficient. */
		return;
	}
	...
	if (( roam_list_find(arg) != NULL) && ( trust_list_find(arg) != NULL))
	{
		debug(LOG_DEBUG, " not need roam.");
		UNLOCK_ROAM_LIST();
		write_to_socket(fd, "No", 2);	/* Error handling in fucntion sufficient. */
	...
	roam_client(arg,T_ROAM_ADD);

	UNLOCK_ROAM_LIST();

	write_to_socket(fd, "Yes", 3);

	debug(LOG_INFO, "Exiting wdctl_roam...");
}

static void wdctl_roamdel(int fd, const char *arg)
{
	
	debug(LOG_INFO, "Entering wdctl_roamdel...");
	if(!is_valid_mac(arg))
	{
		debug(LOG_ERR, "wdctl_roamdel input a invalid MAC address!");
		write_to_socket(fd, "No", 2);	
		return;
	}
    ...
    if ((client_list_find_by_mac(arg)) != NULL)
	{
		debug(LOG_DEBUG, " not need roam del.");
		UNLOCK_CLIENT_LIST();
		write_to_socket(fd, "No", 2);	/* Error handling in fucntion sufficient. */
		return;
	}
...
int auth_roam_del_client_trust(const char *mac)
{
	...
	if (del_from_roamed_mac_list(mac)) 
	{
		debug(LOG_INFO, "del a roamd mac ok!");
		if (!iptables_fw_access_roam(FW_ACCESS_DENY,mac)){
	        rc = 0;
		}
	}
