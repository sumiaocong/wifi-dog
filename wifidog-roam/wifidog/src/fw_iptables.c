...
int
iptables_fw_access_roam(fw_access_t type, const char *mac)
{
    int rc;

    fw_quiet = 0;
    debug(LOG_INFO, "Enter iptables_fw_access_roam()");
    switch (type) {
    case FW_ACCESS_ALLOW:
		debug(LOG_INFO, "iptables adding a roam rule");
		rc =  iptables_do_command("-t mangle -A " CHAIN_TRUSTED " -m mac --mac-source %s -j MARK --set-mark %d", mac,
					FW_MARK_KNOWN);
        break;
    case FW_ACCESS_DENY:

		
		rc =  iptables_do_command("-t mangle -D " CHAIN_TRUSTED " -m mac --mac-source %s -j MARK --set-mark %d", mac,
					FW_MARK_KNOWN);
        break;
    default:
        rc = -1;
