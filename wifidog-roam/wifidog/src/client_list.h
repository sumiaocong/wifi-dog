...
extern pthread_mutex_t roam_list_mutex;
...
//added by chengang@20210312
#define LOCK_ROAM_LIST() do { \
	debug(LOG_DEBUG, "Locking roam list"); \
	pthread_mutex_lock(&roam_list_mutex); \
	debug(LOG_DEBUG, "roam list locked"); \
} while (0)

#define UNLOCK_ROAM_LIST() do { \
	debug(LOG_DEBUG, "Unlocking roam list"); \
	pthread_mutex_unlock(&roam_list_mutex); \
	debug(LOG_DEBUG, "roam list unlocked"); \
} while (0)
